...
test.each([Mode.SAFE, Mode.FAST])(
  ...
  async (mode) => {
    ...
    await expect(async () => await appContainer.get(C)).rejects.toThrow(OutOfScopeResolutionError);
  },
);
...
test.each([Mode.SAFE, Mode.FAST])(
  "finalize all scopes after error in nested scope cleanup generator with async generators",
  async (mode) => {
    let finalizedD = false;
    let finalizedC = false;
    ...
    const provider3 = new Provider(Scope.ACTION);
    provider3.provide(B);
    provider3.provide(A, [B, C], async function* (b, c) {
      yield new A(b, c);
      throw new Error("A cleanup failed");
    });

    const appContainer = makeAsyncContainer(Scope.APP, [provider1, provider2, provider3], mode);

    await expect(async () => {
      await appContainer.withNestedScope(Scope.REQUEST, async (requestContainer) => {
        await requestContainer.withNestedScope(Scope.ACTION, async (actionContainer) => {
          await actionContainer.get(A);
        });
      });
    }).rejects.toThrow(ContainerCleanupError);

    expect(finalizedD).toBeTruthy();
    expect(finalizedC).toBeTruthy();
  },
);

test.each([Mode.SAFE, Mode.FAST])(
  "finalize all scopes after error in nested scope cleanup generator with sync generators",
  async (mode) => {
    let finalizedD = false;
    let finalizedC = false;
    ...
    const provider3 = new Provider(Scope.ACTION);
    provider3.provide(B);
    provider3.provide(A, [B, C], function* (b, c) {
      yield new A(b, c);
      throw new Error("A cleanup failed");
    });
