...
test.each([Mode.SAFE, Mode.FAST])(
  ...
  (mode) => {
    ...
    expect(() => appContainer.get(C)).toThrow(OutOfScopeResolutionError);
  },
);
...
test.each([Mode.SAFE, Mode.FAST])(
  "finalize all scopes after error in nested scope cleanup generator",
  (mode) => {
    ...
    const appContainer = makeSyncContainer(Scope.APP, [provider1, provider2, provider3], mode);
